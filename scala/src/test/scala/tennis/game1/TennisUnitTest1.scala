...
class TennisUnitTest1() {
  ...
  def checkCalculateScoreForMatchPoints(): Unit = {
    val game = new TennisGame1("player1", "player2")
    assertEquals("Advantage player1", game.calculateScoreForMatchPoints(3,2))
    assertEquals("Advantage player2", game.calculateScoreForMatchPoints(2,3))
    assertEquals("Win for player1", game.calculateScoreForMatchPoints(5,1))
    assertEquals("Win for player2", game.calculateScoreForMatchPoints(4,4))
  }
